...
const paintQuestions = () => {
    ...
    if (data.step < dataBase.length) {
        ...
        questions.forEach((question, localIndex) => {
            ...
            html += `<input type="range" name="${slug}-question-${localIndex}" id="${slug}-question-${localIndex}" min="0" max="10" value="${(data.answers.length > data.step && data.answers[data.step][localIndex]) ? data.answers[data.step][localIndex].value : 0}" data-index="${localIndex}" data-slug="${slug}">`;
        ...
        html += 'Se han enviado sus resultados!';
    }
    app.innerHTML = html;
    addEventListeners();
...
const confirmStepComplete = () => {
    return data.answers[data.step].filter(answer => (answer && answer.value > 0)).length === 7;
};
